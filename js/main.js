...
authMod.watchAuthState((user, profile) => {
  ...
  if (!profile) {
    showToast("Signed in, but couldn't reach Firestore to load your profile. Check your connection or any ad-blocker.", "error");
  }
...
function initCreateGameWizard() {
  ...
  qs("#btn-start-game").addEventListener("click", async () => {
    ...
    try {
      const gameId = await gameSetupMod.createGame({
        bankerUid: wizard.bankerUid,
