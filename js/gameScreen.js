...
function renderHeader() {
  ...
  const roleLabel = isBanker ? "You're the Banker" : myPlayerDoc() ? "Player view" : "Observing";
  qs("#game-role-badge").textContent = roleLabel;
  qs("#game-role-badge").classList.toggle("badge-banker", isBanker);
...
function renderOverviewTab() {
  ...
  // Observer: set this game up (createdBy) but isn't a listed player and
  // isn't the Banker — e.g. a parent running the device for two kids.
  // They can see everything but every write action belongs to the Banker,
  // so no quick-action buttons here.
  const players = state.currentGamePlayers;
  const totalCash = players.reduce((s, p) => s + p.cashBalance, 0);
  const activeCount = players.filter((p) => p.status !== "bankrupt").length;
  const mortgagedCount = properties.filter((p) => p.mortgaged).length;
  return `
    ...
    <p class="muted">You set this game up but aren't a player or the Banker — you can watch everything here, but only the Banker can make changes.</p>
  `;
}
